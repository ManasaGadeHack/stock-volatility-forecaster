...
# Enhanced CSS with all new features
st.markdown("""
<style>
    ...
    * { 
    ...
    /* Top nav with live price ticker */
    .top-nav {
    ...
    .metric-block.green { border-color: #10b981; }
...
def calculate_risk_score(predicted_vol):
    """Calculate 1-10 risk score"""
    if predicted_vol < 0.15:
        return 1, "Very Low"
    elif predicted_vol < 0.20:
        return 3, "Low"
    elif predicted_vol < 0.25:
        return 5, "Medium"
    elif predicted_vol < 0.35:
        return 7, "High"
    else:
        return 10, "Very High"
...
if result:
    ...
    <div class="top-nav">
        ...
        <div class="price-ticker">
            <div class="live-price">${result['current_price']:.2f}</div>
            <div class="price-change {price_class}">
                {price_arrow} ${abs(result['price_change']):.2f} ({result['price_change_pct']:+.2f}%)
            </div>
            <div style="color: #6b7280; font-size: 0.9rem;">{result['ticker']} • {datetime.now().strftime('%A, %d %B %Y')}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
else:
...
if result:
    ...
    # 5 metric blocks (added risk score)
    ...
    <div class="metrics-row">
        ...
        <div class="metric-block green">
            <div class="metric-label">Model Accuracy</div>
            <div class="metric-value">94.6%</div>
            <div class="metric-change up">Historical</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # Large chart with confidence intervals
    ...
    <div style='text-align: center; padding: 4rem 0; color: #6b7280;'>
        ...
        <p>Real-time volatility forecasting with confidence intervals • Risk scoring • Export results</p>
    </div>
    """, unsafe_allow_html=True)